...
> Пункты с 4 по 8 - **дополнительны**, их стоит выполнять только в случае, если приложения продолжают определять root (Например, для приложения банка Тинькофф часто требуются пункты 4-7).
...
8. На некоторых прошивках даже с выполнением 1-3 устройство может не проходить проверку Device integrity ([справка](https://developer.android.com/google/play/integrity/verdicts#device-integrity-field), [приложение для проверки](https://play.google.com/store/apps/details?id=krypton.tbsafetychecker)). Для исправления используем модуль [Play Integrity Fix.](https://github.com/chiteroman/PlayIntegrityFix)
